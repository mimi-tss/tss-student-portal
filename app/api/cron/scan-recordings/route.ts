...
export async function GET(req: NextRequest) {
  ...
  // `days` is a manual-only escape hatch (still gated by CRON_SECRET
  // above, never public) for a one-time historical catch-up — e.g. the
  // 2026-09-10 Drive restructure (see MEET_RECORDINGS_INBOX_FOLDER_ID's
  // own comment) went undetected long enough that the ordinary rolling
  // window couldn't reach back far enough on its own once found. Omitted,
  // this is exactly the steady-state call it always was.
