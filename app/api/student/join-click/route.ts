...
// actually belongs to the caller before logging, so the resulting
// evidence can't be polluted with a bogus session_id. Existence of a
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabase
    ...
    .insert({ event_type: "join_click", actor_id: user.id, session_id: sessionId });
