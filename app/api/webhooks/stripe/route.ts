...
export async function POST(req: NextRequest) {
  ...
  switch (event.type) {
    ...
    case "invoice.payment_failed": {
      ...
      if (customerId) {
        await admin.from("students").update({ payment_status: "dnc" }).eq("stripe_customer_id", customerId);
      }
      break;
    }
    ...
    case "invoice.payment_succeeded": {
      ...
      const customerId = typeof invoice.customer === "string" ? invoice.customer : invoice.customer?.id;
      if (customerId) {
        await admin.from("students").update({ payment_status: "ok" }).eq("stripe_customer_id", customerId);
      }
      break;
    }
...
// the table's unique email constraint.
...
// Portal-driven upgrade/downgrade or scheduled cancellation.
async function handleSubscriptionUpdated(admin: AdminClient, subscription: Stripe.Subscription) {
  ...
  if (!student) {
    ...
    console.error("subscription event for unknown stripe_customer_id", customerId);
    return;
  }
  ...
  // Scheduled-cancellation detection — a student who used the Billing
  // Portal to cancel gets cancel_at_period_end flipped true immediately,
  // well before the subscription actually ends (customer.subscription.
  // deleted, handled separately below). Dedup by checking for an
  // existing pending/approved request first, same pattern as
    ...
    if (!existingRequest) {
      ...
      const { data: inserted } = await admin
        ...
        .insert({
          ...
          status: "approved",
          reason: "Cancelled via Stripe Billing Portal.",
      ...
      if (inserted) {
        await createAttentionItem(admin, {
          ...
          summary: `${student.name} cancelled via Stripe Billing Portal · effective end of cycle ${effectiveDate}`,
        });
