...
- The only outbound webhook events Kajabi fires are `order.created` and `payment.succeeded` — confirmed directly from the Kajabi dashboard's own Webhooks screen (Kajabi's docs had described a different, incorrect `purchase.created`/`cart.purchase` split that isn't what the actual UI offers). **There is no webhook for subscription cancellation, pause, or payment failure.**
