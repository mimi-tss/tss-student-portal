...
- **DNC (Do Not Coach) automation — designed, not fully working yet.** Kajabi has no payment-failed webhook (confirmed — see section 1). The plan was for the same 5-minute polling job that catches cancellations to also flag DNC on a failed payment, auto-block upcoming sessions in the coach schedule view, and post to Slack — but building the poll around the real, confirmed `GET /v1/contacts` + offers-relationship mechanism revealed it **can't actually tell a failed payment apart from a real cancellation** (both just show as "offer no longer held"). Today, a lapsed-payment student gets marked `subscription_status: cancelled`, not `payment_status: DNC` — functionally similar (blocks new bookings) but the wrong flag, and no Slack notification. DNC clears automatically once `payment.succeeded` fires (that event does exist).
- **Decided: no automated DNC-vs-cancellation detection will be built.** Instead, admin (who already has to monitor for this) handles a DNC student's already-scheduled session with the same **Staff cancel** tool used for studio-side cancellations, with its **"Issue a session credit" checkbox unchecked** — a DNC student hasn't paid for the session being cancelled, so no credit. Logged as a distinct `admin_overrides.override_type` (`staff-cancel-no-credit` vs. the credited `staff-cancel`) so the two stay separately auditable. This closes the "Real DNC detection" open item as a process decision rather than a code gap — see section 5 for the actual staff-cancel mechanics.
...
**Admin can cancel a session two ways (built):** on the admin per-student view, next to the upcoming session. **"Cancel"** applies the exact same rules as the student's own self-service cancellation (24+ hours notice earns a capped student-fault credit, counts against 1/month or 6/year) — shares the actual decision logic with the self-service route via `lib/booking/cancel-session.ts` rather than duplicating it, with one difference: the self-service route lets the student's own RLS insert policy reject an over-cap credit, while this route checks the cap explicitly first, since admin's own insert policy has no cap of its own to lean on. **"Staff cancel"** is for studio-side reasons (coach emergency, scheduling error, goodwill) — by default issues a credit (uncapped, no expiry, `studio-emergency` type) regardless of notice timing, requires a typed reason, and logs it to `admin_overrides` for audit (matches the "admin can override any makeup restriction ... logged with a required note" language already in this section). **Built: an "Issue a session credit" checkbox (checked by default)** — unchecking it skips crediting entirely, for a DNC/non-paying student's cancellation (section 3's DNC decision) rather than a studio-side one; still requires and logs a reason either way, just under a distinct `override_type` (`staff-cancel-no-credit`) so the two stay auditable separately. Both credited paths (regular cancel and staff cancel with the box checked) reinstate the original credit instead of minting a new one if the session being cancelled was itself booked with one — same reasoning as the self-service reinstatement.
...
**Built: both cancel confirmation dialogs (student's own, and admin's "regular cancel" — not staff-cancel, which is uncapped) show the remaining cap** — "Makeup credit cap remaining: X/1 this month · Y/6 this year" — computed from the same numbers the server enforces (`lib/booking/cancellation-caps.ts`, the single source the display and the enforcement in `cancel-session.ts` both import, so they can't drift apart). Only shown for a non-makeup cancellation, since rescheduling a makeup session just returns the same credit and never touches the cap. **Reason is now required, not optional,** on both the student's own cancel and admin's regular cancel (previously optional on both — only staff-cancel required one) — a deliberate choice to always have a reason on file, not a backend rule change; the API still accepts the field as optional server-side, only the UI enforces it.
...
- **Built: every date/time shown anywhere in the app names its zone explicitly** (e.g. "8:00 PM ET"), never shows seconds, and uses US numeric dates (M/D/YYYY). A header-nav control (student/coach/admin layouts alike) shows the current viewing zone and lets it be changed at any time — persisted per-browser in `localStorage`, not tied to the account, since there's no server-side profile field for it. Defaults: **Eastern** for admin (the studio's own reference zone — "everyone follows that time"); **the coach's own configured zone** for a coach's own dashboard (unchanged from the point above — they live and work in it); **the viewer's browser-detected local zone** for students (falls back to Eastern before the browser detection effect runs, then swaps — no SSR/hydration mismatch). This is purely a display-layer convention — the underlying scheduling engine (working hours, recurring-schedule wall-clock times, `zonedTimeToUtc` conversions) is unchanged and still keyed to each coach's own `timezone`; a recurring schedule's day/time is still entered and stored in the assigned coach's zone, just *displayed* converted into whatever zone the viewer has selected (`lib/scheduling/recurring.ts`'s `nextWeeklySlotInstant` + `lib/timezone.ts`'s `formatTimeInZone`/`formatDateTimeInZone`/`timezoneAbbreviation`). The booking page's existing timezone picker (`lib/timezones.ts`) now feeds the same shared, persisted selection instead of a page-local one that reset between pages.
...
- **Built:** click a student's name on the calendar grid (or from the chat picker) to open `/coach/students/[id]` — a **student snapshot panel**: next session, makeup credit count + expiry, Homework Notes, and chat. Not yet built into this panel: subscription tier detail beyond the plain tier/status line, Google Drive folder link.
- **Built: Homework Notes** — dated running log per student (`homework_notes` table, RLS added migration `0022` — the table existed since day one but had zero policies, so nothing could actually read/write it before now), **visible to the student** on their own dashboard (capped to the 5 most recent with a "Show N more" expand, per the ~5–8-entry guidance — not the full history by default), optionally pinned (shown first, with a 📌 marker). Only real coaches can author a note (`homework_notes.coach_id` is a required FK to `coaches` — admin can view every student's notes but doesn't author through this table, since admin has no coach identity to attach).
- **Built: notes and chat carry over across coach changes, permanently, not just for the currently-assigned coach.** Whichever coach a student is reassigned to (or gets as a one-off substitute — see the admin bullet below) sees the exact same notes/chat history any other coach of theirs would, including everything written by coaches before them — nobody loses access when reassigned, access only ever accumulates (`auth_coach_student_ids()`, i.e. "every student I've ever had a real session with", widened in migration `0022` to also gate chat + notes, not just the `students` table it already gated). This is why the snapshot panel is deliberately the *same* page/data regardless of which coach opens it.
- Chat with assigned students (plus any student they've had a session with before, even if since reassigned — see above), with attachments
...
- **Built: manual substitute-coach assignment** — admin, and only admin, can change which coach handles a **single already-scheduled session** ("Reassign coach" next to Cancel, on both the next-session block and the "Show all sessions" list — `sessions.actual_coach_id`, re-checked against the new coach's own schedule for a clash), which coach runs a student's **weekly recurring schedule** (a coach picker on the schedule form itself, independent of the student's overall assigned coach — e.g. a covering coach can run the regular slot without reassigning the student overall), or which coach a **makeup/credit gets booked with** (a coach picker on admin's book-on-behalf-of page, `/admin/students/[id]/book`). Students and coaches cannot change any of these themselves — enforced at the RLS layer (`recurring_schedules` is admin-`for all`-only; `sessions` UPDATE for a non-admin coach is scoped to their own sessions and can't reassign `actual_coach_id` away from themselves), not just hidden in the UI.
...
- **Built (migration `0022`): access to the thread is additive, not exclusive.** Originally, RLS gated a thread purely on its *current* `coach_id` — re-pointing it to a new coach (on reassignment) meant the old coach lost access outright, even though the trigger above still handed the new coach the full prior history. Now, any coach who's ever had a real session with that student (`auth_coach_student_ids()`) keeps permanent read+write access alongside the current coach, so nobody's shut out by a later reassignment — see section 8's admin bullet on substitute-coach assignment, which is what actually creates these multi-coach situations. Message sender names are resolved from whoever actually sent each message (looked up directly against `coaches`/`students`), not just the thread's current two participants, so a previous coach's older messages still attribute correctly instead of showing "Unknown".
